...
class DecisionTreeClassifierSQLModel(TreeModel):
    ...
    def get_case_sql(self, dbms: str, used_array: dict = None) -> str:
        ...
        def visit_tree(node):
            ...
            feature_name = self.features[node].split('"')[1]
            if((used_array is None) or feature_name not in used_array.keys()):
                sql_dtm_rule = f" CASE WHEN {self.features[node]} {op} {thr} THEN"
            else:
                sql_dtm_rule = f" CASE WHEN {used_array[feature_name]} {op} {thr} THEN"
