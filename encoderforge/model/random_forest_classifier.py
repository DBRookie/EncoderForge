...
from encoderforge.model.decision_tree_classifier import DecisionTreeClassifierSQLModel
from encoderforge.model.base_model import TreeModel
from encoderforge.base.operator import Operator
from encoderforge.base.defs import ModelName,quote_if_str
import encoderforge.base.defs as defs
from encoderforge.cost_model.cost import TreeCost
    ...
    def query(self, input_table: str, dbms: str, used_columns: dict = None, lr_down: list = None, float: bool = False) -> str:
        ...
        for i in range(len(self.decision_tree_classifiers)):
            ...
            if(used_columns is None):
                ...
                sql_case = decision_tree_classifier.get_case_sql(dbms,used_array=used_columns)
        ...
        for class_ix, class_label in enumerate(self.classes):
            ...
            for i in range(len(self.decision_tree_classifiers)):
                quoted_label = quote_if_str(class_label)
                majority_class_query += f"CASE WHEN tree_{i} = {quoted_label} THEN 1 ELSE 0 END + "
            
            majority_class_query = majority_class_query[:-3]  # remove the last ' + '

            majority_class_query += f") AS class_{class_ix}, "

        majority_class_query = majority_class_query[:-2]  # remove the last ', '
        majority_class_query += f" FROM ({query}) AS F"
        ...
        for i in range(len(self.classes)):
            ...
            case_stm += f"\nThen {quote_if_str(self.classes[i])}\n"
